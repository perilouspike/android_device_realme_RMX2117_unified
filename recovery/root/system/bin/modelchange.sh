...
if [ $? -eq 0 ]; then
  ...
  while [ $i -le 10 ]; do
    if [ -f /cache/runtime.prop ]; then
      ...
      if [ "$name" ]; then
        echo "change name"
        sed -i "s/Realme Narzo 30 Pro 5G/${name}/" /prop.default
        sed -i "s/Realme Narzo 30 Pro 5G/${name}/" /default.prop
        sed -i "$ a ro.product.model=${name}" /prop.default
      else
        echo "pass change name"
        sed -i "$ a ro.product.model=Realme Narzo 30 Pro 5G" /prop.default
      fi
